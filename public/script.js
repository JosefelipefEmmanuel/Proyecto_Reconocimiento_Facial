            ...
            if (labeledFaceDescriptors.length > 0) {
                ...
                for (let result of results) {
                    ...
                    if (result.label === 'unknown') {
                        ...
                        // Verificar si tiene permiso vigente
                        const tienePermiso = await verificarPermiso(nombre);
                        if (!tienePermiso) {
                            notifyUser(`❌ Acceso denegado para ${nombre}`, true);
                            return;
                        }

                        // Si tiene permiso, continuar con el registro
                        ...
                        if (userId) {
                            ...
                            if (registerStatus) {
                                notifyUser(`✅ Acceso exitoso para ${nombre}`);
                                showCustomAlert(`✅ Acceso concedido: ${nombre}`);
                            }
...
// Verificar si el usuario tiene permiso autorizado
async function verificarPermiso(nombre) {
    try {
        const response = await fetch('/verificar-permiso', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                nombre: nombre, // 👈 CAMBIO AQUÍ
                empresaId: selectedEmpresaId
            })
        });
