...
// Subir usuario con foto
app.post('/upload', upload.single('photo'), (req, res) => {
    const { name, cedula, cargo, empresaId } = req.body;
    const photo = req.file.buffer;

    db.query("SELECT COUNT(*) AS count FROM tabla_usuarios WHERE cedula = ? AND codigo_empresa = ?", [cedula, empresaId], (err, results) => {
        ...
        db.query("INSERT INTO tabla_usuarios (nombre, cedula, cargo, imagen, codigo_empresa) VALUES (?, ?, ?, ?, ?)", [name, cedula, cargo, photo, empresaId], (err) => {
            if (err) return res.status(500).send('Error al insertar en la base de datos');
...
app.post('/verificar-permiso', (req, res) => {
    const { nombre, empresaId } = req.body;

    const buscarUsuario = `
        SELECT id FROM tabla_usuarios
        WHERE nombre = ? AND codigo_empresa = ?
    `;

    db.query(buscarUsuario, [nombre, empresaId], (err, userResults) => {
        ...
        const verificarPermiso = `
            SELECT * FROM permisos_acceso
            WHERE usuario_id = ? AND vencimiento > NOW() AND autorizado = 1
            LIMIT 1
        `;

        db.query(verificarPermiso, [usuarioId], (err, permisoResults) => {
            if (err) {
                console.error('Error al verificar permiso:', err);
                return res.status(500).json({ autorizado: false });
            }

            const autorizado = permisoResults.length > 0;
            res.json({ autorizado });
        });
    });
});
